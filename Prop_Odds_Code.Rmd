...
cum_log = multiroot(paramfnGen,guesses,plac_prob = placebo,exp_prob = experimental)$root
...
multiroot(paramfnGen,guesses,plac_prob = placeboP1,exp_prob = experimentalP1)$root # Cumulative log odds ratios provided under row P1 of Supplemental Table 3.

# Deriving T0-P3-M0-C0, a placebo group which is less skewed. Consequently, the experimental group is less skewed as well.
...
multiroot(paramfnGen,guesses,plac_prob = placeboP3,exp_prob = experimentalP3)$root # Cumulative log odds ratios provided under row P3 of Supplemental Table 3.
```
...
simFunprop = function(plac_prob,exp_prob){
  ...
  return(cat("Power:", mean(trt_results[,3] > qnorm(0.975))))
}

# Uncomment code to run simulation to derive power for treatment effect scenario T1-P0-M0-C0.
placebo = c(.012,.053,.162,.144,0.364,0.265)
